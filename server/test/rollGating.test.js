...
test("rolling again without doubles is rejected", () => {
  ...
  // From tile 0 (Start), (1,3) lands on tile 4 (ضريبة, a fixed-amount tax
  // tile) -- no buy prompt or random card draw that could otherwise set a
...
test("doubles in free play grants exactly one bonus roll", () => {
  ...
  // (2,2) from tile 0 lands on tile 4 (ضريبة, a fixed-amount tax tile --
  // safe/deterministic). No buy prompt or random card draw to keep the next
  // roll deterministic.
  const first = withDice([[2, 2]], () => room.rollDice("p0"));
  assert.equal(first.doubles, true);
  assert.equal(room.canRollAgain, true);

  // (3,5) from tile 4 lands on tile 12 (the Holding Pen tile itself -- a safe
...
test("three consecutive doubles sends the player to the Holding Pen without moving that roll, and grants no bonus roll", () => {
  ...
  // (2,2) from tile 0 -> tile 4 (ضريبة, safe/deterministic).
  ...
  // (4,4) from tile 4 -> tile 12 (the Holding Pen tile, also safe/deterministic
